...
import com.example.project_movie_backEnd.dto.request.LoginRequest;
import com.example.project_movie_backEnd.dto.request.SignupRequest;
import com.example.project_movie_backEnd.dto.response.JwtResponse;
...
import com.example.project_movie_backEnd.security.services.UserDetailsImpl;
...
//    클라이언트에 인증된 사용자 정보 전송(토큰 + 사용자 정보)
    ...
    public Boolean idOverlapCheck(@PathVariable String username) {
...
//    이메일이 DB 에 있는 지 확인
        if (userRepository.existsByEmail(signUpRequest.getEmail())) {
            return ResponseEntity
                    .badRequest()
                    .body(new MessageResponse("이미 등록된 이메일 입니다."));
        }
        ...
        if (userRepository.existsByPhone(signUpRequest.getPhone())) {
            return ResponseEntity
                    .badRequest()
                    .body(new MessageResponse("이미 등록된 휴대폰 번호 입니다."));
        }
