package com.example.project_movie_backEnd.configuration;

import com.example.project_movie_backEnd.security.auth.CustomOAuth2UserService;
import com.example.project_movie_backEnd.security.auth.OAuth2SuccessHandler;
import com.example.project_movie_backEnd.security.jwt.AuthEntryPointJwt;
import com.example.project_movie_backEnd.security.jwt.AuthTokenFilter;
import com.example.project_movie_backEnd.security.services.UserDetailsServiceImpl;
...
//  아래 함수 정의 안하면 @Autodwired 로 가져올 때 에러가 발생할 수 있음
    @Bean
...
//                .sessionManagement().sessionCreationPolicy(SessionCreationPolicy.NEVER).and()// EVER 옵션은 세션을 사용하진 않지만 만약 필요하다면 세션을 생성해주는 옵션
                .authorizeRequests().antMatchers("/api/**").permitAll() // 이 url 은 모든 사용자 접근 허용
//                .antMatchers("movie/**").permitAll() // 이 url 은 모든 사용자 접근 허용
//                .antMatchers("api/movie/**/**/**").permitAll() // 이 url 은 모든 사용자 접근 허용
                .antMatchers("/api/auth/**", "/auth-redirect").permitAll() // 이 url 은 모든 사용자 접근 허용
